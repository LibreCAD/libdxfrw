...
| I0.3 | I0 / S18 | WP8; cardinality-aware mapping and disposition | I0.2a, I0.2c | ACTIVE | EXPERIMENTAL | total `1:1`/`1:N`/`N:1`/`N:M` target-to-standalone ledger with route references and readiness fields | I0.3-A emits and validates one row for every target route: 4,885 equivalent `1:1`, 737 delta-review `1:1`, eight reviewed signature-adapted `1:1` aliases, two `N:1` DXF range reductions, one `1:N` DXF range split, 33 explicit `1:0` raw-classifier target-debt rows, and 69 standalone-only IDs. Each row has owner paths, implementation/disposition, dependencies, smallest fast gate, fixture policy, and unblock condition. Raw-classifier aliases are not guessed; next work proves their ordered domain partition and converts only eligible debt to reviewed N:* mappings; mapping self-test, pinned `--check`, source/policy gates pass; unblocks I0.4 after zero-unmapped closure |
