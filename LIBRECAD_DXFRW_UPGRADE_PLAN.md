...
- Latest implementation slice: S18/I0.3-A conservative cardinality mapping
  ...
  `1:1` (4,885 equivalent and 737 explicit delta-review rows); 47 target-only
  routes remain visible as `1:0` target debt and 83 standalone-only routes are
  listed separately. Every row carries façade/category ownership, source
  paths, implementation state, disposition, smallest fast gate, fixture policy,
  and an unblock condition. No alias is guessed: N:* mappings stay reserved
  for a reviewed non-overlapping domain proof. Mapping self-tests, pinned
...
- Expanded child-item states: 82 COMMITTED / 28 PLANNED / 1 READY / 2 ACTIVE /
...
- Active work: I0.2b exact parser-publication/callback proof, with its raw
  ...
  I0.2g’s aggregate disposition and I0.3 cardinality mapping remain before
  the inventory can become a parity claim; I0.2c provenance and artifact
  integrity are source-closed and I0.3’s conservative ledger is active while
  its alias/domain reconciliation remains open.
...
| I0.3 | I0 / S18 | WP8; cardinality-aware mapping and disposition | I0.2a, I0.2c | ACTIVE | EXPERIMENTAL | total `1:1`/`1:N`/`N:1`/`N:M` target-to-standalone ledger with route references and readiness fields | I0.3-A emits and validates one row for every target route: 4,885 equivalent `1:1`, 737 delta-review `1:1`, 47 explicit `1:0` target-debt rows, and 83 standalone-only IDs. Each row has owner paths, implementation/disposition, dependencies, smallest fast gate, fixture policy, and unblock condition. No N:* alias is invented; next work normalizes reviewed alias domains (especially DXF group-code classification), proves coverage/no-overlap, and converts eligible debt to `1:N`/`N:1`/`N:M`; mapping self-test, pinned `--check`, source/policy gates pass; unblocks I0.4 after zero-unmapped closure |
