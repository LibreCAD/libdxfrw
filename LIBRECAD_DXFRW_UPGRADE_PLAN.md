...
- Current checkpoint: S75/J51 IMAGEDEF/IMAGEDEF_REACTOR parity is verified and
  ready to commit; S01-S74 implementation slices are committed and S75 is the
  active verification slice with a
...
- Active verification slice: S75/J51 IMAGEDEF/IMAGEDEF_REACTOR parity covers
...
- Next implementation slice: S76/J52 IMAGE AC1015 legacy compatibility is
  ready to trace the target/spec wire layout and either apply a bounded fix or
  record a version-gated unsupported disposition without staging fixtures.
...
- Previous fully resolved slice: S73 (the GEODATA civil-data/version-window
  ...
  terminal only when their recorded gates pass; the next runtime qualification
  lane requires no new target pin.
- Resolved slices: 74/76 (`COMMITTED`, or `SUPERSEDED` after all replacements
  commit).
...
- Parent-item states: 1 READY / 0 PLANNED / 0 ACTIVE / 0 VERIFYING /
  1 VERIFIED / 0 BLOCKED_HARD / 0 SUPERSEDED / 74 COMMITTED.
...
- Active work: S01-S74 are committed; S75/J51 IMAGEDEF/IMAGEDEF_REACTOR
  object-family parity is verified and ready to commit; S76/J52 AC1015
  compatibility is ready after the S75 commit.
  S23/I5
...
| S76 | J52: IMAGE AC1015 legacy compatibility | S75 | READY | ODA/target wire-layout trace; bounded version-gated correction or unsupported disposition; plan/scope/sync/fixture gates | ready to isolate the AC1015 IMAGE/IMAGEDEF/IMAGEDEF_REACTOR failure, compare target and ODA body/handle ordering, and either correct the production path or preserve the explicit unsupported gate without staging fixtures | focused AC1015 writer/self-read and diagnostic probes; no full suite unless a release checkpoint declares it |
...
| J52 | S76 | J51 | READY | EXPERIMENTAL | Isolate and resolve the AC1015 IMAGE/IMAGEDEF/IMAGEDEF_REACTOR legacy wire-layout failure against the pinned target and ODA specification, applying only a bounded correction or retaining an explicit unsupported disposition with trace evidence |
...
| J52.1 | J52 / S76 | WP5, WP7, WP8, WP10; IMAGE AC1015 compatibility | J51 | READY | EXPERIMENTAL | trace AC1015 body and handle-stream failure with the ODA spec and pinned LibreCAD implementation, run the smallest production correction that preserves newer-version behavior, or document a version-gated unsupported result without staging fixtures | focused AC1015 diagnostics and self-read/oracle checks; any unresolved legacy discrepancy remains bounded and non-promoting |
