...
- Latest implementation slice: S18/I0.2d transport-selection proof now binds
  each ASCII/binary/R12 reader/writer constructor to its exact predicate and
  direct branch arm, with synthetic inversion rejection. It builds on the
  exact raw-publication and read-side eligibility metadata; raw writer/replay,
  DWG lifecycle, coverage, and mapping edges remain later work.
...
- Last fully resolved slice: S17 (standalone evidence closure is committed in
  `e41f10e946335e89151302b2fa6327052fad5470`); target integration commit is
  `6969e0a003414f9a7084349ac54bc2b32515e16b`. S18 remains ACTIVE while its
  independent source-only slices land and self-update this block.
...
- Active work: I0.2b exact parser-publication/callback proof, with its raw
  callback terminal metadata slice landed, and I0.2d read-side eligibility/
  transport selection proof landed. I0.2d (DXF
...
| S18 | I0: deterministic both-façade parity inventory | S17 | ACTIVE | pinned generator inputs, source/public-surface and pipeline-edge extraction, cardinality-aware mapping, zero-unmapped/duplicate checks, deterministic `--check` | source/package integration, raw-publication terminal proof, read-side raw eligibility, and DXF ASCII/binary/R12 transport-selection evidence are committed; target and standalone routes include exact carrier/callback/guard/branch/predecessor metadata, while raw writer/replay, DWG lifecycle, coverage, and mapping work remain before this becomes a parity claim; no drawing payloads | S19 |
...
| I0.2d | I0 / S18 | WP4; DXF transport and raw eligibility graph | I0.2 | PLANNED | EXPERIMENTAL | source-proven ASCII/binary/R12 transport selection, virtual provider closure, record-scope transaction classification, and ordered raw boundary/depth/payload/handle/limit gates; read-side eligibility and transport branch evidence are landed, with writer replay identity still open | target-only/synthetic checks mutate every read/readAscii/write selection and raw eligibility predicate; reject transport construction without its guard, inherited virtual operation gaps, and record-scope-as-transport; no fixtures; unblocks I0.2g/I0.2c |
