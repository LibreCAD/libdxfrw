...
| **I0.2e-A / I0.2g-A — receipt is not delivery / role is not coverage** | Table receipt, deferred frame, journal, or a functional file role cannot close a capability on its own.  The eight table maps, block-record path, compound queues, direct/journal delivery, lifecycle finalizers, and every functional source unit require non-self concrete evidence. | **Landed in S18 in two source-only slices:** raw-section ingress, base unsupported hook, R2004+ acceptance/buffer checks, and R2004/R2007 finalizer flush loops have ordered source evidence; all nine table descriptors now separately prove control parse → handle claim → deferred receipt stage → typed record parse → map/publication insertion, with `kBlockTable` kept distinct. Remaining work is BLOCK/ENDBLK ownership, ATTRIB/SEQEND/INSERT/MINSERT/VERTEX/POLYLINE compound delivery, direct/journal paths, and reader lifecycle closure; then make I0.2g reject a generic stage-only or foreign-path `coveredBy` link. Double-generate before each artifact refresh. | Keeps broad coverage as one cached aggregate check after focused source slices, rather than serializing all 85 units; raw-section finalizer and table-delivery ownership are independent fast gates. |
...
- Latest implementation slice: S18/I0.2e-A DWG table-delivery identity now
  binds all nine descriptors independently (LTYPE, LAYER, STYLE, DIMSTYLE,
  VPORT, BLOCK, APPID, VIEW, UCS) through control parsing, handle claiming,
  deferred control-receipt staging, typed record parsing, and map/publication
  insertion. `kBlockTable` remains explicitly distinct so its map can feed the
  later BLOCK/ENDBLK ownership walk. This builds on the committed DWG raw
  section lifecycle and raw-object replay identity plus the DXF proof; compound
  entity transitions, reader lifecycle, mapping, and format qualification
  remain later work. The slice is source-only, fail-closed, and self-checked
  before commit.
...
- Active work: I0.2b exact parser-publication/callback proof, with its raw
  ...
  versioned acceptance/buffering → R2004/R2007 finalizer flush; and the latest
  I0.2e-A slice proves the nine table receipt/parse/map chains with an explicit
  block-table boundary. I0.2d (DXF transport/raw eligibility), I0.2e (DWG
  lifecycle/table/compound delivery), I0.2f (writer bridge/lifecycle), and
  I0.2g (concrete source-unit closure) remain separate implementation slices
  so false generic source edges cannot survive as a claim of feature parity.
  I0.2a now proves the exact 85-unit pinned source
...
| S18 | I0: deterministic both-façade parity inventory | S17 | ACTIVE | pinned generator inputs, source/public-surface and pipeline-edge extraction, cardinality-aware mapping, zero-unmapped/duplicate checks, deterministic `--check` | source/package integration, raw-publication terminal proof, read-side raw eligibility, DXF ASCII/binary/R12 transport selection, DXF raw writer/replay transforms, DWG raw replay ingress/preflight/internal-registration/owner-bookkeeping identity, DWG raw section ingress/buffer/finalizer identity, and all-nine-descriptor table receipt/parse/map identity are committed; target and standalone routes include exact carrier/callback/guard/branch/predecessor metadata plus ordered DWG replay/table edges, while broader DWG BLOCK/compound delivery, reader lifecycle coverage, and mapping remain before this becomes a parity claim; no drawing payloads | S19 |
