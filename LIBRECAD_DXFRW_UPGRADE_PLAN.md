...
| **I0.2b-C / I0.2d-A — raw-route callback omission** | `raw-route` is outside the current parser-publication category, so generic raw-flow terminals must not be treated as publication proof.  The proxy paths physically publish typed proxy callbacks and then `addRawDxfEntity`/`addRawDxfObject`; each physical raw callback needs callback route, carrier binding/form, source location, ordered condition ancestry, and raw-flow predecessor edge. | **Landed in the S18 source-only slice:** validated terminal-call metadata covers `addRawDxfObject`, `addRawDxfEntity`, and `addRawDxfSection` across the five target/standalone raw publication bodies; proxy rows independently prove typed-callback-before-raw-carrier order and branch compatibility. Synthetic mutations reject missing raw callbacks, ambiguous bindings, incompatible callback routes, reversed proxy order, and dangling predecessors. The remaining I0.2d-A work is the ordered record/section boundary, depth, payload, self-handle, and pair-limit eligibility graph. | Separates the completed DXF publication terminal proof from the still-open eligibility graph; I0.2b remains ACTIVE until its broader typed/template/DWG obligations close, while I0.2d-A is the next narrow implementation slice. |
...
- Latest implementation slice: S18/I0.2b raw-publication terminal proof now
  emits exact source-owned callback metadata for `processRawObject`,
  `processRawEntity`, `processProxyObject`, `processProxyEntity`,
  `processRawDxfSection`, and the pinned standalone counterparts. Proxy rows
  retain typed-callback-before-raw-carrier order; the ordered boundary/depth/
  payload/handle/limit eligibility graph remains an I0.2d-A gate.
...
- Active work: I0.2b exact parser-publication/callback proof, with its raw
  callback terminal metadata slice landed. I0.2d (DXF
...
| S18 | I0: deterministic both-façade parity inventory | S17 | ACTIVE | pinned generator inputs, source/public-surface and pipeline-edge extraction, cardinality-aware mapping, zero-unmapped/duplicate checks, deterministic `--check` | source/package integration and the raw-publication terminal slice are committed; target and standalone routes now include exact raw carrier/callback/ancestry/predecessor metadata for the five DXF raw publication bodies, while ordered eligibility, DWG lifecycle, writer, coverage, and mapping work remain before this becomes a parity claim; no drawing payloads | S19 |
