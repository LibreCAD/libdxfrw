...
- Latest implementation slice: S18/I0.2e-B DWG compound transition identity
  now binds the concrete `readDwgEntityWithOutput` case arms for ATTRIB,
  SEQEND, INSERT/MINSERT, VERTEX variants, and POLYLINE variants to their
  versioned aggregate/staging helpers (`stagePending*`, mapped/legacy insert
  and polyline paths). The route metadata keeps the compound arms separate
  from ordinary `entryParse` publication and fails closed on helper/order
  changes. This builds on the committed table-delivery and raw-section/object
  lifecycle slices plus the DXF proof; direct/journal delivery internals,
  BLOCK/ENDBLK ownership walk, reader lifecycle, mapping, and format
...
- Active work: I0.2b exact parser-publication/callback proof, with its raw
  ...
  table receipt/parse/map chains with an explicit block-table boundary; and
  I0.2e-B now binds the compound entity case arms to their versioned staging
  helpers. I0.2d (DXF transport/raw eligibility), I0.2e (DWG
...
| S18 | I0: deterministic both-façade parity inventory | S17 | ACTIVE | pinned generator inputs, source/public-surface and pipeline-edge extraction, cardinality-aware mapping, zero-unmapped/duplicate checks, deterministic `--check` | source/package integration, raw-publication terminal proof, read-side raw eligibility, DXF ASCII/binary/R12 transport selection, DXF raw writer/replay transforms, DWG raw replay ingress/preflight/internal-registration/owner-bookkeeping identity, DWG raw section ingress/buffer/finalizer identity, all-nine-descriptor table receipt/parse/map identity, and compound ATTRIB/SEQEND/INSERT/MINSERT/VERTEX/POLYLINE transition identity are committed; target and standalone routes include exact carrier/callback/guard/branch/predecessor metadata plus ordered DWG replay/table/compound edges, while broader DWG BLOCK/ENDBLK ownership, direct/journal delivery, reader lifecycle coverage, and mapping remain before this becomes a parity claim; no drawing payloads | S19 |
