...
| **I0.2d-B / I0.2f-A — raw transport/replay identity** | Helper-name anchoring cannot conflate `requiresDxfSelfHandle(Version)` with `requiresDxfSelfHandle(dxfReader)`.  `writeRawDxfObject` is not terminal: it calls `writeRawDxfGroups`.  Internal `registerRawObjectClass` during DWG replay is not external writer ingress. | **DXF portion landed in S18:** reader/writer ASCII/binary/R12 selection rows carry exact predicate, branch-arm, and constructor evidence; `validateRawDxfGroups`, writer self-handle overloads, and `writeRawDxfObject`/`writeRawDxfSection` → `writeRawDxfGroups` transforms carry concrete call-site IDs and predecessor order. Remaining work is the raw-DWG ingress→OR-preflight→single chosen branch→internal registration chain; mutate each edge independently. | DXF transport and raw replay are now fast independent gates; DWG writer identity remains a parallel narrow scan, so no writer implementation waits on DWG compound work. |
...
- Latest implementation slice: S18/I0.2d-B DXF raw writer/replay proof now
  binds `validateRawDxfGroups`, the version/self-handle overloads, and
  `writeRawDxfObject`/`writeRawDxfSection` → `writeRawDxfGroups` transforms at
  their concrete call sites. It builds on exact publication, eligibility, and
  transport metadata; DWG raw replay preflight, lifecycle, coverage, and
  mapping edges remain later work.
...
- Active work: I0.2b exact parser-publication/callback proof, with its raw
  ...
  transport selection, and DXF raw writer/replay proof landed. I0.2d (DXF
...
| S18 | I0: deterministic both-façade parity inventory | S17 | ACTIVE | pinned generator inputs, source/public-surface and pipeline-edge extraction, cardinality-aware mapping, zero-unmapped/duplicate checks, deterministic `--check` | source/package integration, raw-publication terminal proof, read-side raw eligibility, DXF ASCII/binary/R12 transport selection, and DXF raw writer/replay transforms are committed; target and standalone routes include exact carrier/callback/guard/branch/predecessor metadata, while DWG replay/lifecycle, coverage, and mapping work remain before this becomes a parity claim; no drawing payloads | S19 |
