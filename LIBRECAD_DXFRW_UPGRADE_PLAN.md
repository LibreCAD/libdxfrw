...
| J362 / S386 | Replace J295 with the existing six-version local-from-scratch generator, J360, pinned LibreDWG full JSON, and an explicit AC1024 high-bit-classes case.  The ODA-range correction now preserves the RL-declared CRC byte and a production-independent raw parser recomputes stored CRC `0x443e` with zero reader/dropped diagnostics.  Immutable v1 was frozen before the oracle, validated 12/16 fields, and correctly required a successor for LibreDWG `pt[0..2]` plus radians; immutable v2 then predeclared those mappings and its `1e-12`-degree normalization before a clean rerun. | `libdxfrw_dwg_local_roundtrip`, reader-matrix and diagnostic tests; both immutable pinned-oracle field contracts; J360 target/standalone differential; mutation checks for schema, units, tolerance, provenance, discrepancy set, and diagnostic debt.  Generated DWG/JSON stays temporary. | V2 validates 16/16 predeclared claims with zero exclusions, binds all 34 oracle outcomes and the exact J360 17,304/ten-path result, and needs no successor.  Self-read remains reachability only; evidence is field-scoped and non-promoting until J363 freezes eligible claims/status and J364 supplies hosted native receipts. |
...
- Current checkpoint (2026-09-16): S334/J310 hardens transient DXF parser
  state across public-model copies and assignments.  LWPOLYLINE vertex-count
  admission, ATTRIB subclass routing, and GEOPOSITIONMARKER repeated-code
  counters now reset for a fresh parser walk while persisted payloads remain
...
- Current checkpoint (2026-09-16): S336/J312 extends transient parser-state
  isolation to MLINE copies and assignments.  Segment/vertex walk cursors and
  count markers now reset while persisted multiline geometry remains copied;
  focused hardening vectors pass and no drawing bytes or derived fixtures are
  used.

- Current checkpoint (2026-09-16): S337/J313 extends transient parser-state
  isolation to TABLE copies and assignments.  Subclass routing and grid
  dimension cursors now reset while persisted table content and style fields
...
- Current checkpoint (2026-09-16): S339/J315 extends transient parser-state
  isolation to UNDERLAY copies and assignments.  Clip-vertex indices and
  inverse-clip count state now reset while persisted underlay geometry
  remains copied; focused hardening vectors pass and no drawing bytes or
  derived fixtures are used.

- Current checkpoint (2026-09-16): S340/J316 extends transient parser-state
  isolation to NAVISWORKSMODEL copies and assignments.  Transform-component
  cursors and subclass/body markers now reset while persisted flags, handles,
  transforms, and unit factors remain copied; focused hardening vectors pass
  and no drawing bytes or derived fixtures are used.

- Current checkpoint (2026-09-16): S341/J317 extends transient parser-state
  isolation to POINTCLOUD copies and assignments.  Body/subclass and declared
  count markers now reset while persisted point-cloud metadata, source files,
...
- Current checkpoint (2026-09-16): S343/J319 extends transient parser-state
  isolation to NURBSURFACE copies and assignments.  Coordinate masks and
  subtype markers now reset while persisted NURBS flags and vectors remain
  copied; focused hardening vectors pass and no drawing bytes or derived
  fixtures are used.

- Current checkpoint (2026-09-16): S344/J320 extends transient parser-state
  isolation to REVOLVEDSURFACE copies and assignments.  Transform cursors and
  class-ID markers now reset while persisted surface fields remain copied;
  focused hardening vectors pass and no drawing bytes or derived fixtures are
  used.

- Current checkpoint (2026-09-16): S345/J321 extends transient parser-state
  isolation to EXTRUDEDSURFACE copies and assignments.  Transform cursors,
  class-ID/data-size markers, and subtype state now reset while persisted
  surface fields remain copied; focused hardening vectors pass and no drawing
  bytes or derived fixtures are used.

- Current checkpoint (2026-09-16): S346/J322 extends transient parser-state
  isolation to SWEPTSURFACE copies and assignments.  Transform/entity-ID,
  data-size, and subtype markers now reset while persisted surface fields and
  ACIS payloads remain copied; focused hardening vectors pass and no drawing
  bytes or derived fixtures are used.

- Current checkpoint (2026-09-16): S347/J323 extends transient parser-state
  isolation to LOFTEDSURFACE copies and assignments.  Transform/reference
  counters and subtype markers now reset while persisted surface fields and
...
- Current checkpoint (2026-09-16): S349/J325 extends transient parser-state
  isolation to SECTIONOBJECT copies and assignments.  The AcDbSection
  subclass-body marker now resets while persisted section geometry and
...
- Current checkpoint (2026-09-16): S351/J327 extends transient parser-state
  isolation to BREAKDATA copies and assignments.  The AcDbBreakData
  subclass-body marker now resets while persisted point-reference and
  dimension-handle data remain copied; focused hardening vectors pass and no
  drawing bytes or derived fixtures are used.

- Current checkpoint (2026-09-16): S352/J328 extends transient parser-state
  isolation to DICTIONARY and DICTIONARYWDFLT copies and assignments.  Pending
  entry names and default-entry markers now reset while persisted dictionary
...
- Current checkpoint (2026-09-16): S354/J330 extends transient parser-state
  isolation to SORTENTSTABLE copies and assignments.  Body, block-owner, and
  pending-entity markers now reset while persisted draw-order handles remain
  copied; focused hardening vectors pass and no drawing bytes or derived
  fixtures are used.

- Current checkpoint (2026-09-16): S355/J331 extends transient parser-state
  isolation to INDEX, IDBUFFER, LAYERINDEX, and SPATIALINDEX copies and
  assignments.  Timestamp/body and pending-entry markers now reset while
  persisted timestamps, index entries, handles, and opaque data remain copied;
  focused hardening vectors pass and no drawing bytes or derived fixtures are
  used.
...
- Current checkpoint (2026-09-16): S357/J333 extends transient parser-state
  isolation to DATALINK copies and assignments.  Body, custom-data, adapter,
  and pending-handle markers now reset while persisted connection metadata and
  custom rows remain copied; focused hardening vectors pass and no drawing
  bytes or derived fixtures are used.

- Current checkpoint (2026-09-16): S358/J334 extends transient parser-state
  isolation to GEODATA copies and assignments.  Subclass and mesh cursor
  markers now reset while persisted geolocation metadata and mesh payloads
  remain copied; focused hardening vectors pass and no drawing bytes or
  derived fixtures are used.

- Current checkpoint (2026-09-16): S359/J335 extends transient parser-state
  isolation to SPATIALFILTER copies and assignments.  Matrix-value staging is
  reset while persisted clip geometry, distances, and transform matrices remain
  copied; focused hardening vectors pass and no drawing bytes or derived
  fixtures are used.

- Current checkpoint (2026-09-16): S360/J336 extends transient parser-state
  isolation to EVALUATIONGRAPH copies and assignments.  Body, node/edge, and
  malformed-state markers now reset while persisted graph values remain copied;
  focused hardening vectors pass and no drawing bytes or derived fixtures are
  used.
...
- Current checkpoint (2026-09-16): S362/J338 extends transient parser-state
  isolation to POINTCLOUDCOLORMAP copies and assignments.  Ramp selection and
  default-scheme cursors now reset while persisted class metadata, schemes,
  and ramp vectors remain copied; focused hardening vectors pass and no drawing
  bytes or derived fixtures are used.

- Current checkpoint (2026-09-16): S363/J339 extends transient parser-state
  isolation to SUNSTUDY, MOTIONPATH, CURVEPATH, and POINTPATH copies and
  assignments.  Date/hour, positional, and handle cursors now reset while
...
- Current checkpoint (2026-09-16): S365/J341 extends transient parser-state
  isolation to SECTION settings copies and assignments.  Body routing and
  type/geometry indices now reset while persisted section settings and raw
  payloads remain copied; focused hardening vectors pass and no drawing bytes
  or derived fixtures are used.

- Current checkpoint (2026-09-16): S366/J342 extends transient parser-state
  isolation to BACKGROUND and NAVISWORKSMODELDEF copies and assignments.
  Overloaded 90/290 and repeated-status cursors now reset while persisted
  background/model fields remain copied; focused hardening vectors pass and no
  drawing bytes or derived fixtures are used.

- Current checkpoint (2026-09-16): S367/J343 extends transient parser-state
  isolation to LAYOUT copies and assignments.  The embedded PlotSettings and
  Layout subclass marker now resets while persisted plot/layout fields and
...
- Current checkpoint (2026-09-16): S369/J345 extends transient parser-state
  isolation to ASSOCIATIVEOBJECT copies and assignments.  The persistent
  subentity-manager body marker and class-value counter now reset while
  persisted associative payloads and prefix statuses remain copied; focused
  hardening vectors pass and no drawing bytes or derived fixtures are used.

- Current checkpoint (2026-09-16): S370/J346 is a post-hardening validation
  checkpoint.  The complete dependency-free CTest topology passes 31/31 in
  5.13 seconds after the parser-state isolation slices; differential,
...
- Current checkpoint (2026-09-16): S372/J348 closes the configuration gap from
  S371.  A fresh pinned LibreCAD system-package build with `BUILD_TESTS=ON`
  defines and builds `libdxfrw_system_fast_tests` plus
  `librecad_filter_compile_check`; the system fast test passes all 176
  assertions in 12 cases.  This validates the installed public package only;
  native-platform, long-fuzz, external-oracle, and format-support promotion
  evidence remain open.  No drawing bytes or derived fixtures are changed.

- Current checkpoint (2026-09-16): S373/J349 reconciles release documentation
  with the live support ledger.  `docs/UPGRADE_SUPPORT.md` now distinguishes
  dispatch, decode, publication, read/write support, raw preservation,
  derived rendering, and validation; records 1,345 DWG and 1,475 DXF routes
  with zero qualified/advertised rows; documents the 176-assertion system fast
  consumer proof; and states the CMake/C++17 2.0.0 plus deprecated-legacy-
...
- Current checkpoint (2026-09-16): S375/J351 hardens MULTILEADER entity copy
  and assignment semantics.  Persisted context, style, handle, and legacy
  array values are copied while the transient DXF nested-context state resets;
  focused hardening vectors pass and no drawing bytes or derived fixtures are
  used.

- Current checkpoint (2026-09-16): S376/J352 hardens DIMARC and LARGE_RADIAL
  dimension copy and assignment semantics. Persisted arc/jog values are
...
- Current checkpoint (2026-09-16): S378/J354 extends the proxy-graphics/ACIS
  regression through the public `dxfRW::readAscii` façade.  The callback sink
  confirms 3DSOLID, REGION, BODY, and PLANESURFACE publication preserves the
...
- Current checkpoint (2026-09-16): S383/J359 makes that retirement
  reproducible.  `metadata/external-evidence-resolution-v1.json` now binds all
  49 observed hashes, 50 safe aliases, four distinct set digests, the sole
  duplicate, magic-derived formats/versions, public/repository/unresolved
  provenance, audited ref-tip snapshots, and exact admission dispositions.
  J268 remains explicitly reconstructed with no hash membership or digest;
  all six historical rows remain `DEFERRED_EXTERNAL`, archived, and
  non-promoting.  The fail-closed checker dynamically joins J284 to both
  21-row reports and exactly three admitted fixtures, while the advisory
  runner now classifies content before suffixes, deduplicates before limits,
  retains aliases, separates 29 paths/28 streams/24 DWGs/four non-DWGs, and
  signs its deterministic generation receipt.  Both focused CTest entries and
  the live private-corpus audit pass; no drawing bytes or generated outputs
  enter Git.  S384 is unblocked for the normalized semantic differential.

- Current checkpoint (2026-09-16): S384/J360 supplies the normalized semantic
  differential rather than another conversion-status aggregate.  One C++17
  adapter source is compiled against the pinned LibreCAD target archive and
  standalone package; the receipts prove identical effective build profiles,
  exact static-link closure, and the same fail-closed 170-callback
  `DRW_Interface` inventory digest.  Schema v2 validates ordered callbacks,
  typed finite fields, record/handle/owner/block relationships, opaque carriers,
  unsupported-content dispositions, diagnostics, first failure, deterministic
  execution, locked-input provenance, and independently detected input/output
  format and version.  A clean Release qualification run passes 7/7 focused
  tests.  DXF read/write and DWG read/write reports have zero mismatches and
  respectively 160, 1,116, 32,595, and 2,477 exact compared values, but remain
  `reviewedNonPromoting` because 2, 14, 519, and 46 unsupported-content entries
  are explicitly excluded.  Thus no support claim changes.  Existing admitted
  repository fixtures are read by provenance, locally generated outputs stay
  temporary, and no drawing payload enters Git.  S385/J361 and S386/J362 are
  independently ready.

- Current checkpoint (2026-09-16): S384a/J360.3 closes the expected-failure
  qualification gap found by the post-commit audit.  Expected failures now
  require one unique manifest-bound input/facade/direction case and exact
  first-failure stage/code/path; an oracle-free or merely side-equal failure
  is rejected.  A live deterministic DXF write probe uses the locked local
  recipe and an already-existing output directory, requires both adapters to
  report `write` / `BAD_OPEN` (`2`) / `/output`, and verifies that the
  directory and sentinel remain intact.  The two sides produce 146 exact
  values and zero mismatches, but the report remains
  `reviewedNonPromoting`/ineligible by construction.  Schema/runner and build
  helper self-tests plus the focused live CTest pass; no drawing payload is
  added.  S385/J361 and S386/J362 remain independently ready.

- Current checkpoint (2026-09-16): S386/J362.1 resolves the AC1024 extended
  CLASSES-footer integrity blocker.  Both R2004-lineage and R2007 readers had
  first computed the correct CRC byte as `20 + RL class-data size`, then
  overwritten it with the end of the string-only view; for the 536,312-bit
  extended footer this interpreted high-size word `0x0010` as the CRC.  The
  readers now retain the RL-declared boundary while keeping the smaller bound
  only for string parsing.  A test-only raw-page parser reconstructs the
  temporary AC1024 CLASSES section, independently reads `RL=73839`,
  `bitSize=590705`, `maxClassNum=1328`, and the extended string footer, then
  computes ODA CRC-16 bit-by-bit with seed `0xC0C1` and polynomial `0xA001`;
  stored and computed CRC both equal `0x443e`.  Local AC1021/24/27/32 reads
  report zero CLASSES CRC mismatches, AC1024 drops zero diagnostics, and the
  roundtrip/reader-matrix/diagnostic plus admitted-fixture focused tests pass.
  All generated drawings remain temporary.  J362.2 is now READY as S386a and
  must still obtain independently parsed, predeclared field evidence.

- Current checkpoint (2026-09-16): S385/J361 replaces J284's aggregate
  conversion result with bounded independent semantic evidence on exactly the
  three already-admitted LibreCAD blobs.  Pinned LibreDWG 0.14 full JSON,
  executable/formula/bottle/library hashes, an exact two-artifact non-system
  closure (`4170272163a077731dad3b4d77602dbf38d32f3232c877b135d6b67e0895f1b0`),
  Darwin/arm64/26.5.2 identity, and a fixed loader/debug/config-free
  environment are bound to contract
  `3c0c67ae7a37716946d42f7b08a52c38bc3bbed3ee28b8b6c43f1537d2ec93d2`.
  Thirteen candidate claims are explicit: two LARGE_RADIAL identity-only,
  four exact plus four `1e-12`-tolerance MPOLYGON fields, and three exact
  RTEXT fields.  LARGE_RADIAL and ARCALIGNEDTEXT payloads plus MPOLYGON color
  remain excluded.  Shared manifest
  `d383e9af7e5fb4732b3d90b51c00dea44657e9240f1b91bf88773179a47412e8`
  scopes reviewed debt by input ID/path/hash, facade, direction, option digest,
  exact JSON pointer, value digest, and diagnostic context: the local DWG
  writer has ten exact target-only legacy-CRC diagnostic rows, and each
  admitted fixture has one exact message-digest row.  The writer and fixture
  reports have respectively 2,467; 15,023; 13,308; and 32,594 exact values,
  zero mismatches, and remain `reviewedNonPromoting`; all 9/9 focused
  qualification tests and adversarial self-tests pass.  No drawing or oracle
  JSON payload enters Git.  S386a/J362.2 is the sole READY local evidence lane.

- Current checkpoint (2026-09-16): S386a/J362.2 executes the immutable
  pre-oracle AC1024 candidate contract without repairing it after observation.
  The frozen contract digest is
  `e9f6d18df974e7cbc23701ad906fe33e4ca6542fa24861bef9b5d86a7a4764f0`;
  the local-from-scratch input digest is
  `6acb7d4a0289d200d3dd448abc77082250f1678ec2ad815870e4617fa16abffd`.
  Twelve of sixteen field claims agree across target, standalone, and pinned
  LibreDWG.  Four RTEXT claims fail closed: LibreDWG exposes insertion as
  `pt[0..2]` rather than the frozen `ins_pt[0..2]`, and exposes rotation in
  radians while the two libdxfrw adapters expose degrees.  All 34 LibreDWG
  discrepancy outcomes and the exact 17,304-value J360 comparison (ten
  expected target-only diagnostic paths) are dependency- and digest-bound;
  mutation tests reject any extra difference or altered diagnostic context.
  The result is deliberately `new-child-required` with
  `promotesSupport=false`.  S386b/J362.3 is now READY to freeze the corrected
  `pt` mapping and radians-to-degrees normalization before a fresh run.  No
  generated DWG, DXF, or oracle JSON payload enters Git.

- Current checkpoint (2026-09-16): S386b/J362.3 closes the four exact
  successor conditions without mutating S386a.  Contract v2 was frozen before
  the rerun at
  `7b7baf1351e418cc2aa91faf4f4750e9eeeeff31a03518571e5be05e59f667dd`;
  it binds the committed v1 manifest/checker and bounded v1 observations,
  predeclares LibreDWG `pt[0..2]`, and converts oracle radians to semantic
  degrees with a fixed `1e-12`-degree tolerance derived from the predecessor's
  JSON quantum plus sixteen binary64 ulps.  The fresh run validates all 16
  claims with zero exclusions and no successor condition.  It preserves the
  same generated-input/oracle/dependency closure, binds all 34 oracle
  discrepancy outcomes at `ff6767de…400a`, and repeats the exact J360 result:
  17,304 equal values plus the ten already-bound target-only diagnostic paths.
  The contract remains field-scoped and `promotesSupport=false`; no fixture-,
  version-, or whole-format claim is inferred.  OFF/live focused CTests and
  mutation checks pass, no generated payload is retained, and S387/J363 is
  now READY to freeze immutable claims, workflow, and implementation digests.

- Current checkpoint (2026-09-16): S387a/J363.0 lands the non-promoting
  native-qualification framework and fixes the focused CTest topology before
  freezing anything that must name its generating commit.  The draft ledger
  contains exactly 15 field claims across two routes and advertises none:
  eleven S385 AC1032 claims remain `EXPERIMENTAL` behind their recorded
  integrity/target-debt blockers, while four S386b AC1024 RTEXT claims
  (insertion x/y/z and radians-to-degrees rotation at `1e-12` degrees) are
  independently eligible but remain `EXPERIMENTAL` only until the workflow,
  broad inventory, and implementation digests are frozen.  The exact seven
  `native-qualification-focused` tests pass 7/7 in a fresh Release hosted-
  profile equivalent.  Draft claims/status/input manifests, the strict join
  checker, receipt schema/emitter, exact-artifact/API verifier, and local
  qualification runner all pass their adversarial self-tests; every freeze
  state remains `DRAFT_WORKFLOW_AND_DIGEST_PENDING`, every final digest is
  null, and no receipt or support promotion is fabricated.  No DWG, DXF, or
  oracle payload is retained.  S387/J363.1 is now READY to generate the broad
  inventory from this committed topology, freeze the workflow and both
  digests, and move only the four eligible rows to `PENDING_NATIVE`.

- Current checkpoint (2026-09-16): S387/J363.1-J363.2 completes the local
  fail-closed qualification authority.  The exact 38-test broad inventory is
  frozen against topology commit
  `4b6df4b78d0d2a74feb7caa1e7355ab20703ab5b` with name digest
  `0b84147089615530148d6bc0f0239eec0592b2e19f3a34c9ee0b58c614a1ef14`;
  its exact seven focused tests pass 7/7 in both single- and multi-config
  Release builds.  Workflow SHA-256
  `72c3c75af1d3a8836564bc11ac695d787f9c8fe0f9e132b8c0571d3542f51473`
  pins full action SHAs, Ubuntu 24.04/GCC 13, macOS 15 arm64/AppleClang, and
  Windows 2022/VS17, with per-platform Python, LF-stable full-history
  checkouts, exact pre-seal/sealed artifact sets, and API-verifiable receipts.
  The immutable claims digest is
  `7da9d990ab3d2b2ea18784a26a348a055085b16020519822c4deb543db1230f9`;
  the 165-input implementation digest is
  `0a0a30a69d479a871d04028a0bec52ec61658f8297634ff03f7731cdd64c17b9`.
  Exactly four independently eligible AC1024 RTEXT field claims are now
  `PENDING_NATIVE`; eleven AC1032 claims retain their integrity/target-debt
  blockers, and both aggregate routes remain `EXPERIMENTAL` with zero
  advertised support.  Promotion now requires digest recomputation plus
  complete authenticated live replay of exactly three focused and three
  final-broad receipts; individual field promotion cannot advertise an
  incomplete route.  Strict manifest/workflow/digest/support/pre-native
...
- Current checkpoint (2026-09-17): replacement PR #93 run `35176840834` at
  head `3521f37a1efb9de66dbbf46781b89702d8593d60` is non-promoting.  The
  workflow contract passed and macOS/Clang completed its focused build and
  receipt path, but Linux/GCC stopped on `-Werror=misleading-indentation` at
  the three one-line coordinate-bound updates in `src/drw_acis.cpp:533-535`;
  no focused receipt is accepted and all receipts from this run remain
  invalid.  S389/J364.3's digest `d8cae87cd5c72469d5aba2897230437d8c4285419b2c0bf2c2956f85d5cb281c`
  is invalidated by the source repair; the replacement implementation digest
  is `a0960bffed1b544ca86d474b2f91b5a2dcbd8c50c975360757925e14c4fa6a89` over
  165 inputs.  Claims digest, support rows, immutable claims, and fixtures
  remain unchanged.  S390/J364.4 is `ACTIVE` to repair the GCC warning and
  rerun native qualification; S388/J364.1 remains `VERIFYING`.

- Current checkpoint (2026-09-17): S390's replacement PR #93 run
  `35177211341` at head `9d5ff3406465c693ce37ff791d22512f4b2feab6` again
  passed the workflow contract but Linux/GCC stopped during the focused build.
  The next diagnostics are two enum/non-enum conditionals in
  `src/drw_entities.cpp:3820,3830`, deprecated-copy assignments for
  `DRW_Dimension` in `src/drw_entities.h`/`src/drw_entities.cpp:20128,20151`,
  and six `-Werror=ignored-qualifiers` casts in
  `src/drw_objects.cpp:10688,10689,11167,11168,12920,12921`.  No focused
  receipt is accepted; the `a0960bffed1b544ca86d474b2f91b5a2dcbd8c50c975360757925e14c4fa6a89`
  digest is invalidated by this source repair.  The replacement implementation
  digest is `55ea027c241237450dde7ccc215245ed596ef7564c8dcf4c65e3e4a87578dc4f`
...
- Current checkpoint (2026-09-17): S393's hosted PR #93 run `35179556627` at
  merge head `073481661990167d92fe18d235918c5f89af2d60` passed the workflow
  contract and all seven focused tests on Linux/GCC and macOS/Clang, including
  receipt emission and enforcement.  Windows/MSVC compiled all focused targets
  and passed six of seven tests, but `libdxfrw_dwg_local_roundtrip` failed
  because `DwgDxfOutputTransaction` attempted `MoveFileExW` while its CRT
  exclusive descriptor was still open; no receipt is accepted.  S393's digest
  `a6fc2c2b23bfd08ce2384db070628ad805229a1e08da09db113640b3be9ffe74` is
  invalidated by the transaction source repair; replacement digest is
  `e64e8f2aa0e7c5aaae009a2f9cb542d283ff7e2589c9acba006408cda76c68c9` over
  165 inputs.  Claims digest, support rows, immutable claims, and fixtures
  remain unchanged.  S393/J364.7 is `COMMITTED`; S394/J364.8 is `ACTIVE` for
  the Windows atomic-publish repair; S388/J364.1 remains `VERIFYING`.

- Current checkpoint (2026-09-17): S394's replacement PR #93 run
  `35180493247` at merge head `073481661990167d92fe18d235918c5f89af2d60`
  confirms the Windows descriptor-close repair compiles and reaches focused
  execution, but `libdxfrw_dwg_local_roundtrip` still fails 1119 assertions
  across versions 13-18 while Linux/GCC and macOS/Clang pass all seven
  focused tests.  The failure is isolated to the Windows execution path; no
  receipt is accepted.  Digest
  `e64e8f2aa0e7c5aaae009a2f9cb542d283ff7e2589c9acba006408cda76c68c9` is
  invalidated by diagnostic instrumentation; replacement digest is
  `ac2200c7c8390bfa4b0909b0cd8f5635451122ed222e1b93cc42992798a7dc15` over
  165 inputs.  Claims digest, support rows, immutable claims, and fixtures
  remain unchanged.  S394/J364.8 is `COMMITTED`; S395/J364.9 is `ACTIVE` to
  expose the writer diagnostic needed for the next Windows repair;
  S388/J364.1 remains `VERIFYING`.

- Current checkpoint (2026-09-17): S395's diagnostic PR #93 run
  `35181235487` confirmed `open-failure` for every Windows local DWG write;
  S396/J364.10 replaces `_wopen` with explicit `_wsopen_s(..., _SH_DENYNO,
  ...)` so the second `ofstream` can open the exclusive temporary.  The
  diagnostic-only slice is committed; digest
  `ac2200c7c8390bfa4b0909b0cd8f5635451122ed222e1b93cc42992798a7dc15` is
  invalidated and replacement digest is
  `4779fa1f812e235caccee44c199231bae1035bd335d089a2f025a450f0c08687` over
  165 inputs.  Claims digest, support rows, immutable claims, and fixtures
  remain unchanged.  S395/J364.9 is `COMMITTED`; S396/J364.10 is `ACTIVE` to
...
- Current checkpoint (2026-09-17): S397's hosted PR #93 run `35182619433`
  reports `stream=1 good=1 identity=0 errno=0 gle=0` (with later Windows
  versions reporting `errno=2`), proving that stream creation and opening
  succeed while the CRT `_fstat64`/`_wstat64` identity tuple rejects the same
  file.  S398/J364.12 replaces that Windows-only comparison with native
  `GetFileInformationByHandle` volume-serial/file-index matching while keeping
  the POSIX descriptor-relative path unchanged.  The implementation digest is
  recomputed as
  `a50d6a63e3daa7ad56f52b2689e9b54f046618a7d98c49654a704c81faef1e41` over 165
  inputs; claims digest, support rows, immutable claims, and fixtures remain
  unchanged.  S397/J364.11 is `COMMITTED`; S398/J364.12 is `ACTIVE` pending
  focused local validation and the replacement hosted run; S388/J364.1 remains
  `VERIFYING` and no native receipt is accepted.

- Current checkpoint (2026-09-17): replacement PR #93 run `35183487882` proves
  the native identity repair now publishes Windows outputs and passes six of
  seven focused tests; Linux/GCC, macOS/Clang, and the workflow contract pass.
  The remaining failure is confined to AC1015/format 15 self-read (40 semantic
  assertions), with the hosted log reporting `DXF file error: format 15 error
  11`.  No receipt is accepted.  S398/J364.12 is `COMMITTED`; the existing
  test now prints the bounded `dwgRW` self-read diagnostic and S399/J364.13 is
  `ACTIVE` to identify the AC1015 reader stage before changing behavior.  The
  replacement implementation digest is
  `51db6eb4cd8cfa3e7d3ee68fbfe40e97226748171a07d2378818e929bdf53773` over 165
  inputs; claims digest, support rows, immutable claims, and fixtures remain
  unchanged.  S388/J364.1 remains `VERIFYING` and J364.2 remains planned.

- Current checkpoint (2026-09-17): S401's replacement PR #93 run
  `35185631469` confirms the AC1015 trace: the published `13565`-byte file is
  parsed in different entity orders on libstdc++ and MSVC, and the MSVC order
  leaves two compound frames unresolved before `readDwgEntities` returns
  false.  S402/J364.16 makes the final `ObjectMap` sweep deterministic by
  selecting the lowest source ordinal (then offset/handle), preserving the
  existing deferred-state semantics without adding fixtures or changing
  support claims.  No receipt is accepted.  S401/J364.15 is `COMMITTED`;
  S402/J364.16 is `ACTIVE`.  The replacement implementation digest is
  `cdcd2733565bdc42e047a04ff69f499781986a84847ce1e491d549c3b9e761d7` over
  165 inputs; claims digest, support rows, immutable claims, and fixtures remain
  unchanged.  S388/J364.1 remains `VERIFYING` and J364.2 remains planned.
...
- Current checkpoint (2026-09-17): S399's replacement PR #93 run
  ...
  Linux/GCC, macOS/Clang, and the other six focused tests pass.  S400/J364.14
  adds a bounded output probe (published byte size and six-byte magic) to the
  existing failure log so the next repair can distinguish truncation from an
  AC1015 section-layout mismatch without retaining any drawing payload.  No
  receipt is accepted.  S399/J364.13 is `COMMITTED`; S400/J364.14 is `ACTIVE`.
  The replacement implementation digest is
  `40dad0535f55f5af15f5a8ac73fc755566e668f874070d14c29de6c8ef64278c` over 165
  inputs; claims digest, support rows, immutable claims, and fixtures remain
  unchanged.  S388/J364.1 remains `VERIFYING` and J364.2 remains planned.
...
- Current checkpoint (2026-09-16): S330/J306 HATCH boundary graph ownership
  is committed.  Explicit copies now clone parser-emitted LINE, ARC, ELLIPSE,
  SPLINE, and LWPOLYLINE boundary edges, preserve HATCH fill/seed state, and
  reject unsupported polymorphic edges; focused hardening vectors and all
  metadata/policy gates are green with no drawing fixtures.
...
- Current checkpoint (2026-09-16): S328/J304 base-entity ownership hardening
  ...
  mutable XDATA variants; representative implicit-derived `DRW_Line` copy and
  assignment vectors pass without drawing fixtures.
...
- Current checkpoint (2026-09-16): S326/J302 public-model ownership hardening
  ...
  entity-owned XDATA variants instead of aliasing mutable `shared_ptr` values;
  hardening vectors cover copy, assignment, move, embedded MText, and mutation
  isolation without adding drawing bytes.  The focused hardening target and
  fast policy gates are green; the next queue is evidence-gated runtime/oracle
  and release closure.

- Current checkpoint (2026-09-16): S306/J282 expanded the target/package DWG
  differential over 21 additional committed LibreCAD testdata files, including
  seven AC1032/R2018 cases. Against target `0ffb38d790db53116978e01fcaf94cee9bc6ee48`
  and external corpus commit `3c028612dd8e75d98692ac346635c190500eea94`, all
  21 source/output/semantic comparisons are equal. The evidence is retained as
  hashes and summaries only; no drawing bytes were copied or committed. This
  advisory lane does not promote independent support claims; native-platform,
  long-fuzz, and release closure remain open.

- Current checkpoint (2026-09-16): S307/J283 made the expanded advisory report
  self-checking. `check_target_advisory_differential.py` validates provenance,
  row-derived relation counters, exact 21-row equality, hash/size fields, and
  tamper-negative self-tests; the CTest hook is metadata-only and never reads
  or stages drawing payloads.

- Current checkpoint (2026-09-16): S308/J284 ran LibreDWG 0.14 as an
  independent advisory reader over the same 21 top-level committed LibreCAD
  testdata DWGs. All 21 conversions succeeded under the ten-second bound;
  the report retains only source/output hashes, sizes, versions, and status,
...
- Current checkpoint (2026-09-16): S310/J286 adds a cross-platform CI lane
  covering Ubuntu, macOS, and Windows. Each runner configures C++17 with docs
  disabled and dependency-free tests enabled, builds Release (including the
  `/bigobj` MSVC library branch), and runs the complete CTest topology. This
  schedules the unavailable native-platform evidence without weakening local
  gates or adding drawing payloads; hosted execution remains required before
  that claim can be promoted.

- Current checkpoint (2026-09-16): S311/J287 adds an opt-in extended fuzz
  target while leaving the default fast target unchanged. With
  `LIBDXFRW_BUILD_LONG_FUZZ=ON`, deterministic in-memory malformed-input
  coverage expands to 8,192 generic vectors, 65,536 DXF vectors, and 16,384
  DWG vectors. The release run passes in 0.15 seconds and ASan/UBSan in 1.86
...
- Current checkpoint (2026-09-16): S313/J289 exercised the exact CI configure
  flags locally on the single-config macOS generator (`CMAKE_BUILD_TYPE=Release`,
  docs off, tests on). The clean build and full 30/30 CTest topology pass in
  5.74 seconds; this confirms the workflow command shape while retaining
  hosted Windows/MSVC execution as the native-platform gate.
...
- Current checkpoint (2026-09-16): S303/J279 route-inventory generator
  ...
  refreshed against target `0ffb38d790db53116978e01fcaf94cee9bc6ee48`, and the
  support matrix records 85 standalone-only routes with zero target-unmapped
  rows. Route check, parity aggregate, support matrix, release-readiness,
  fixture-admission, import-scope, and diff gates pass; no drawing bytes were
  added. Target-bound differential debt and independent qualification remain
  open.
...
- Current checkpoint (2026-09-15): S300/J276 support-documentation refresh
  is committed. `docs/UPGRADE_SUPPORT.md` now reflects the S296 TU framing
  result (both declared-length conventions are accepted and independently
  aligned), while external support promotion remains gated. Plan, release,
  fixture, scope, sync, parity, speed, and diff checks pass; no source or
  drawing bytes changed.
...
- Current checkpoint (2026-09-15): S296/J272 TU terminator framing
  ...
  an independent probe, so length-includes-terminator and
  length-excludes-terminator forms both stay aligned. The focused six-version
  writer matrix, DWG hardening, local round-trip, reader-matrix, and external
  AC1021 conversion checks pass; no drawing bytes changed or were added.
- Previous checkpoint (2026-09-15): S295/J271 plan-counter reconciliation is
...
- Previous checkpoint (2026-09-15): S293/J269 post-BLOCK compatibility full
  ...
  reader/writer matrices, local round trips, hardening, fixtures, parity, and
  release gates. No drawing bytes changed or were added.
- Previous checkpoint (2026-09-15): S292/J268 baseline compatibility sweep is
...
- Earlier checkpoint (2026-09-15): S259/J235 bounded DXF parser fuzz smoke is
  ...
  evidence.
- Latest safety slice (2026-09-15): S260/J236 bounded DWG parser fuzz smoke
  is committed. A deterministic 512-input in-memory corpus spans AC1015,
  AC1018, AC1021, AC1024, AC1027, and AC1032 headers with arbitrary and
  zero-filled tails; every input passes through `dwgRW::readBuffer` without
...
- Latest state-isolation slice (2026-09-15): S262/J238 resets the reusable
  DXF header state at both file-backed and in-memory read entry points. A
...
- Latest cumulative checkpoint (2026-09-15): S264/J240 passes fresh normal
  and ASan/UBSan C++17 builds with all 26 dependency-free CTest entries green
  in 5.92s and 12.62s respectively (macOS leak detection disabled). The
  checkpoint includes the parser fuzz lanes and DXF/DWG state-isolation fixes;
  no drawing bytes were added.
- Latest resource-hardening slice (2026-09-15): S265/J241 adds a configurable
  aggregate DXF read-record budget. Every physical record, including ignored
  comments, consumes the per-session ceiling; exhaustion fails closed with a
  structured `ResourceLimit` diagnostic while preserving the legacy
...
- Latest DXF feature-parity slice (2026-09-15): S267/J243 completes the
  modern MULTILEADER DXF context carrier. Nested root/leader-line break pairs,
  content transformation values, context and line hard-pointer handles, and
  entity-level style/arrow handles now round-trip through the ASCII writer and
  parser. A fixture-free in-memory regression covers markers, geometry,
  handles, and all sixteen transform values under release and ASan/UBSan
  hardening; no drawing bytes were added.
- Latest MULTILEADER safety slice (2026-09-15): S268/J244 adds writer-side
  validation for modern MULTILEADER context payloads. Root, line, point, and
  break collections are bounded; numeric coordinates, context fields, and
  matrix values must be finite; oversized or non-finite input is rejected
...
- Latest MULTILEADER string-boundary slice (2026-09-15): S271/J247 extends
  writer preflight to bound context text and legacy block-label strings at the
...
- Latest target/package differential evidence (2026-09-15): S273/J249 runs
  the pinned bundled-target JSON dumper and the staged standalone-package
  dumper over all 15 locked repository fixtures using identical input paths.
  Fourteen fixtures are byte/JSON identical. The only delta is
  `ordinary_enc_AC1021.dwg`: the pinned target publishes zero entities while
  standalone publishes the fixture's expected three `LINE` records. The
  standalone result is retained because its AC1021 page-map compatibility
  fix is spec- and fixture-backed; this is recorded as target debt rather than
...
- Latest differential-schema slice (2026-09-15): S275/J251 extends the
  target/package harness report with independent byte, semantic, and optional
  coarse status/error-stage relations. Formatting-only output changes are now
  distinguishable from semantic deltas, while absent status fields remain
  explicitly `not-reported`; the self-test covers byte-different/semantic-equal
  output and true semantic mismatch without storing drawing bytes.
- Latest local DWG differential slice (2026-09-15): S276/J252 runs the six
  local-from-scratch writer-version outputs through the pinned bundled target
  and installed standalone package with the S275 relation schema. AC1015,
  AC1018, AC1021, and AC1024 are byte/semantic identical; AC1027 and AC1032
  are reviewed deltas where the target emits only two baseline entities and
...
- Latest differential-aggregate slice (2026-09-15): S278/J254 adds
  deterministic top-level byte, semantic, and coarse-status relation counts
  to the schema-2 report. The locked 15-fixture run records 14 equal and one
...
- Latest differential-debt slice (2026-09-15): S281/J257 adds
  `tools/check_differential_debt.py` and the metadata-only
  `metadata/differential-debt-v1.json`. The checker validates the pinned target
  commit, source/output/semantic hashes, exact reviewed-delta cardinality, and
...
- Latest local debt-ledger slice (2026-09-15): S282/J258 adds the scoped
  ...
  are committed.
- Latest normal checkpoint (2026-09-15): S283/J259 rebuilds the current tree
  and runs all 28 dependency-free CTest entries after the differential/debt
  wave. Every test passes in 6.19s, including the new semantic differential and
  debt self-tests; no drawing fixtures or derived payloads are added, and
  sanitizer/native-platform evidence remains on its declared cadence.
- Latest full-corpus advisory slice (2026-09-15): S284/J260 runs all 29
  available `/Users/dli/doc/dwg` inputs with the current converter. Eighteen
  convert, ten fail, and one times out; all nine AC1024 inputs convert
  successfully, so the previously recorded AC1024 class-parser failure is not
  reproduced here. The result is external-only, hash/status metadata in
  `/private/tmp/libdxfrw-s284-external-all.json`, and does not promote support
  or admit any drawing bytes.
- Latest sanitizer checkpoint (2026-09-15): S285/J261 rebuilds the ASan/UBSan
  hardening target and runs all 28 dependency-free sanitizer CTest entries
...
- Latest oracle-contract correction (2026-09-15): S288/J264 separates the
  ...
  promoted.
- Latest JSON-oracle qualification hardening (2026-09-15): S289/J265 makes
  JSON qualification fail closed when the version, required entity set, or
  bounded entity counts do not match. The runner now records `mismatch`
  instead of unconditionally labelling a structurally incomplete JSON result
  `qualified`; a negative self-test locks this behavior. Existing six-version
  local evidence remains unchanged, with JSON qualifying all six versions and
  no payloads retained.
- Plan-state reconciliation (2026-09-15): S290/J266 closes the three I0
  inventory children whose source/public closure evidence was already delivered
  by the pinned inventory slice but whose rows remained `VERIFIED`. This
  reconciliation is metadata-only: it changes no source or fixture bytes and
  records the exact prior evidence plus current aggregate/policy checks.
...
- Previous checkpoint (2026-09-15): S256/J232 output-transaction final-rename
  ...
  synchronized before and best-effort after rename. A deterministic regression
  that moves the original parent directory fails closed without publishing to
  either the moved or replacement path. Windows retains the documented
  `MoveFileExW(..., MOVEFILE_WRITE_THROUGH)` boundary, and native Windows
  race/durability validation remains a release-platform follow-up. No drawing
  bytes were added.
- Previous checkpoint (2026-09-15): S255/J231 output-transaction parent-directory
  durability hardening is committed. POSIX commits verify that the containing
  directory can be opened and synchronized before pathname publication and
  issue a best-effort post-rename `fsync`; Windows retains
  `MOVEFILE_WRITE_THROUGH` as its native boundary because portable directory
  handles are unavailable. A late post-rename sync failure cannot roll back an
  already visible replacement, so the public contract remains atomic
  visibility plus best-effort crash durability rather than an unconditional
  crash-durable guarantee. No drawing bytes were added.
- Previous checkpoint (2026-09-15): S254/J230 deterministic support-matrix
...
- Latest implementation slice: S249/J225 output-transaction temporary-name
  hardening is committed. POSIX creation now uses `mkstemp` and the Windows
  path uses randomized exclusive names; parallel transactions are covered for
...
- Latest implementation slice: S251/J227 output-transaction file-flush
  hardening is committed. POSIX `fsync` and Windows `FlushFileBuffers` run on
  the retained descriptor before rename; focused transaction tests remain
  green. Parent-directory durability and final rename semantics remain
  unpromoted. No drawing fixtures or derived payloads were added.
- Latest implementation slice: S252/J228 implementation-speed and validation
  cadence research is committed. A schema-checked timing baseline records
  incremental/fast/full build-test durations, a fail-closed changed-path
...
- Previous fully resolved slice: S100 (the NAVISWORKSMODEL entity parity slice is committed
  ...
  `6969e0a003414f9a7084349ac54bc2b32515e16b`; all in-horizon lanes are
  terminal only when their recorded gates pass.
- Resolved slices: 400 (`COMMITTED`).
- Slice states: 0 READY / 0 PLANNED / 1 ACTIVE / 1 VERIFYING / 0 VERIFIED /
  0 BLOCKED_HARD / 0 SUPERSEDED / 400 COMMITTED.
...
- Expanded child-item states: 0 READY / 1 PLANNED / 1 ACTIVE / 1 VERIFYING /
  0 BLOCKED_HARD / 0 VERIFIED / 0 SUPERSEDED / 505 COMMITTED; no child is
  anonymous.
...
- Active work: S01-S384 and J329/J329.1/J330/J330.1/J331/J331.1/J332/J332.1/
  ...
  J345/J345.1/J346/J346.1/J347/J347.1/J348/J348.1/J349/J349.1/J350/J350.1/J351/J351.1/J352/J352.1/J353/J353.1/J354/J354.1/J355/J355.1/J356/J356.1/J357/J357.1/J358/J358.1/J359/J359.1/J359.2/J360/J360.1/J360.2/J360.3/J361/J361.1/J362/J362.1/J362.2/J362.3/J363/J363.0/J363.1/J363.2 and S384a/S385/S386/S386a/S386b/S387a/S387/S389/S390/S391/S392/S393/S394/S395/S396/S397/S398/S399/S400/S401 are committed.  S388/J364.1 is VERIFYING after the hosted runs exposed cross-platform compile defects; S402/J364.16 is ACTIVE for the deterministic entity sweep, and J364.2 remains planned until a complete focused receipt set is accepted.
...
| S12 | F1: per-version/per-feature writer qualification | S11 | COMMITTED | self-read for implemented paths; independent oracle for each `PROMOTED` row; every unqualified row explicitly deferred/experimental | S12 committed with F1.1-F1.5 complete; F1.1a is now experimental after adaptive TU framing coverage; independent support promotion remains gated; no fixture bytes | S13, S14 |
...
| S316 | J292: make progress prose invariant | S315 | COMMITTED | remove volatile branch-count prose; plan check/report; no fixture admission | execution refresh now records the rebase/no-behind invariant and delegates the exact ahead distance to the post-commit report, preventing every subsequent plan commit from making the prose stale; no source or drawing bytes changed | native-platform, protected external fuzz, independent semantic oracle, and release closure |
...
| S342 | J318: POINTCLOUDEX parser-state copy isolation | S341 | COMMITTED | POINTCLOUDEX copy/assignment body/cropping parser-state reset; focused hardening target; fixture admission; import scope; sync; plan; diff | focused `libdxfrw_hardening_tests` passes for copied and assigned POINTCLOUDEX models; no drawing bytes or derived fixtures | continue with target-bound differential debt, independent-oracle, native-platform, and release closure |
| S343 | J319: NURBSURFACE parser-state copy isolation | S342 | COMMITTED | NURBSURFACE copy/assignment coordinate/subclass parser-state reset; focused hardening target; fixture admission; import scope; sync; plan; diff | focused `libdxfrw_hardening_tests` passes for copied and assigned NURBSURFACE models; no drawing bytes or derived fixtures | continue with target-bound differential debt, independent-oracle, native-platform, and release closure |
| S344 | J320: REVOLVEDSURFACE parser-state copy isolation | S343 | COMMITTED | REVOLVEDSURFACE copy/assignment transform/class-ID parser-state reset; focused hardening target; fixture admission; import scope; sync; plan; diff | focused `libdxfrw_hardening_tests` passes for copied and assigned REVOLVEDSURFACE models; no drawing bytes or derived fixtures | continue with target-bound differential debt, independent-oracle, native-platform, and release closure |
...
| S357 | J333: DATALINK parser-state copy isolation | S356 | COMMITTED | DATALINK copy/assignment body/custom-data/adapter/pending-handle parser-state reset; focused hardening target; fixture admission; import scope; sync; plan; diff | focused `libdxfrw_hardening_tests` plus plan, fixture, import-scope, sync, route, aggregate, support, release-readiness, speed, and diff gates pass; no drawing bytes or derived fixtures | commit the verified slice with trailers, report progress, and recompute the next ready queue |
...
| S383 | J359: durable advisory-evidence resolution registry | S382 | COMMITTED | per-hash basis/provenance/admission/ref-tip registry/checker; magic-before-suffix and dedup-before-limit hygiene; separate path/unique/DWG counts; runner self-test; plan; fixture; diff | registry/checker binds 49 hashes and 50 aliases, preserves four observed set digests plus J268's null reconstruction, joins 21 J284 rows and three admissions, and rejects promotion fields; runner live audit reports 29 paths/28 streams/24 DWGs/four non-DWGs/four mislabeled paths with a verified receipt; both focused CTests pass; no drawing bytes | prepare and commit with matching trailers, report progress, then activate S384 |
| S384 | J360: normalized semantic differential harness | S383 | COMMITTED | one dual-build schema-v2 adapter; mandatory ordered callbacks/typed fields/ownership/opaque carriers/unsupported/diagnostics; build/link/interface identities; independent format/version detection; deterministic build/run targets; focused tests; plan; fixture; diff | clean Release build and 7/7 qualification CTests pass; receipts share one effective-build digest and one exact 170-callback interface digest; four deterministic live reports have zero mismatches and remain explicitly non-promoting because unsupported content is excluded; no drawing payload is added | report the committed slice, then execute S385 and S386 in parallel without refreshing any external corpus |
| S384a | J360: exact expected-failure oracle correction | S384 | COMMITTED | manifest-bound expected-failure input/facade/direction/stage/code/path; unique case and registered-input joins; preserved existing-directory live probe; schema/helper/focused CTest; plan; fixture; diff | wrong, duplicate, unregistered, or oracle-free failure cases fail closed; both adapters deterministically report `write`/2/`/output` with 146 exact values and zero mismatches while the destination marker survives; result remains explicitly non-promoting; no drawing payload | report the corrective commit, then execute S385 and S386 in parallel |
| S385 | J361: admitted-fixture independent candidate fields | S384a | COMMITTED | three admitted DWGs; pinned full-JSON field contracts and oracle closure; exact invocation/options debt selectors; graph/preservation/error checks; focused tests; plan; fixture; diff | 13 field-scoped claims pass independently: nine exact and four tolerance-normalized; all three fixture reports have one exact reviewed CRC-message debt and zero mismatches; the local writer's ten legacy target-only diagnostic paths are exactly bound; all results remain non-promoting and no drawing payload is added | report the committed slice, then execute S386a against the final shared-manifest digest |
| S386 | J362: AC1024 extended-classes integrity | S384a | COMMITTED | six-version runtime generator; AC1024 extended class-string footer; independent raw-page/footer/CRC parser; structured zero-CRC/dropped-diagnostic gate; focused roundtrip/reader/diagnostic/fixture tests; plan; fixture; diff | readers retain the RL-declared CRC byte instead of the string-view bound; AC1024 raw evidence binds RL 73839, bitSize 590705, class 1328, string size 536312, and stored/independent CRC 0x443e; all focused gates pass with no drawing payload | report the committed integrity slice, then execute S386a in parallel with S385 |
| S386a | J362: local-from-scratch AC1024 independent candidate fields | S386 | COMMITTED | predeclared field allowlist; pinned full-JSON LibreDWG closure; normalized differential; stable discrepancy bindings; focused tests; plan; fixture; diff | frozen digest …4764f0 validates 12/16 claims; four RTEXT insertion/rotation claims are exact fail-closed exclusions because the oracle uses `pt` and radians; all 34 oracle discrepancies plus 17,304 J360 values/ten expected diagnostic differences are bound; non-promoting and no payload retained | report the committed frozen probe, then execute S386b with a new pre-oracle contract |
...
| S390 | J364.4: repair GCC warning contract in ACIS bounds extraction | S389 | COMMITTED | GCC `-Werror=misleading-indentation` in `src/drw_acis.cpp:533-535`; focused native build; qualification digest/status; plan; fixture; diff | replacement PR #93 run `35176840834` reproduces the three GCC diagnostics; split coordinate-bound conditionals preserve bounds semantics; digest `a0960bffed1b544ca86d474b2f91b5a2dcbd8c50c975360757925e14c4fa6a89` is historical and superseded by later repair; no claim or fixture bytes changed | split each coordinate-bound update into an independently indented conditional, run the smallest focused compile/tests, update digest/status and plan, commit with matching trailers, push to PR #93, then rerun S388 |
| S391 | J364.5: repair remaining GCC/MSVC warning contract | S390 | COMMITTED | GCC enum/non-enum, deprecated-copy, and ignored-qualifier diagnostics in `src/drw_entities.cpp`, `src/drw_entities.h`, and `src/drw_objects.cpp`; focused native build; qualification digest/status; plan; fixture; diff | replacement PR #93 run `35177211341` reproduces the exact diagnostics; explicit sentinel conversions, dimension assignment, and enum cast repairs preserve behavior; digest `55ea027c241237450dde7ccc215245ed596ef7564c8dcf4c65e3e4a87578dc4f` is historical and superseded by later repair; no claim or fixture bytes changed | add explicit enum sentinel conversions, default the `DRW_Dimension` copy assignment, remove const-qualified local enum-value casts, run the smallest focused compile/tests, update digest/status and plan, commit with matching trailers, push to PR #93, then rerun S388 |
| S392 | J364.6: repair remaining hosted handle sentinels and reader-matrix seam | S391 | COMMITTED | GCC enum/non-enum handle sentinels in `src/libdxfrw.h`/`src/libdxfrw.cpp`; Windows/MSVC private-test seam link failure in `tests/dwg_reader_matrix_tests.cpp`; focused native build; qualification digest/status; plan; fixture; diff | hosted run `35177810798` reproduces the four GCC diagnostics and the Windows unresolved `dwgRW::openBuffer`; public `readBuffer` dispatch coverage and explicit sentinel conversions preserve behavior; digest `3230cb388a2aeff7219533a1f7fcff3af89f04657b208c2620c4ee70829ff32e` is historical and superseded by later repair; no claim or fixture bytes changed | convert all four `DRW::NoHandle` fallbacks to `std::uint32_t`, move the matrix test to the public read path with initialized `dx_iface`/`dx_data`, link `dx_iface.cpp`, run the smallest focused compile/tests, update digest/status and plan, commit with matching trailers, push to PR #93, then rerun S388 |
...
| S402 | J364.16: make legacy entity sweep deterministic | S401 | ACTIVE | AC1015 cross-platform entity/frame ordering; focused native build; qualification digest/status; plan; fixture; diff | select the lowest `sourceOrdinal` (then source offset/handle) instead of `unordered_map::begin()` when sweeping `ObjectMap`; preserve deferred compound ownership semantics and no-claim/no-fixture policy; implementation digest `cdcd2733565bdc42e047a04ff69f499781986a84847ce1e491d549c3b9e761d7` is staged | run focused local round-trip plus fast policy gates, commit/push the repair, inspect Windows, and continue automatically if another deterministic defect appears |
...
| J292 | S316 | J291 | COMMITTED | EXPERIMENTAL | Remove volatile branch-count wording | Replace the literal ahead count with a stable rebase/no-behind statement and retain exact count in post-commit reports without changing source or fixture content |
...
| J319 | S343 | J318 | COMMITTED | EXPERIMENTAL | Harden NURBSURFACE parser-state copy and assignment | Reset transient NURBSURFACE coordinate masks and subtype markers across copy/assignment while preserving persisted NURBS flags and vectors; keep the no-fixture policy |
| J320 | S344 | J319 | COMMITTED | EXPERIMENTAL | Harden REVOLVEDSURFACE parser-state copy and assignment | Reset transient REVOLVEDSURFACE transform cursor and class-ID marker across copy/assignment while preserving persisted surface fields; keep the no-fixture policy |
...
| J333 | S357 | J332 | COMMITTED | EXPERIMENTAL | Harden DATALINK parser-state copy and assignment | Reset transient DATALINK body, custom-data, adapter, and pending-handle markers across copy/assignment while preserving persisted connection metadata and custom rows; keep the no-fixture policy |
...
| J359 | S383 | J358 | COMMITTED | NOT_APPLICABLE | Make archival disposition and corpus hygiene reproducible | Add a metadata-only per-hash basis/provenance/admission/ref-tip registry/checker, explicitly prevent an exact J268 membership/digest, classify and deduplicate content before limits, and enforce `archivedNonPromoting` plus `supersededBy` for J256/J260/J268/J284/J293/J295 without changing their historical dispositions |
| J360 | S384 | J359 | COMMITTED | EXPERIMENTAL | Implement the normalized semantic differential required for support evidence | One schema-v2 adapter now compiles against target and standalone packages with exact source/config/binary/static-link/build/interface identities; 170 callbacks are inventory-checked, provenance and format/version are independently bound, deterministic live read/write comparisons have zero mismatches, exact manifest-bound failure semantics are enforced by S384a, and every report remains non-promoting while exclusions or an expected failure exist |
...
| F1.1a | F1 / S12 | P7.5, WP5 | F1.1 | COMMITTED | EXPERIMENTAL | TU terminator length contract between writer and reader | the local ODA specification defines TU as a bitshort length followed by N UTF-16 characters; S296/J272 adds an independent-cursor probe so both length-includes-terminator and length-excludes-terminator streams remain aligned, with codec-backed six-version semantic vectors and truncation safety; independent external support promotion remains gated; unblocks F1.4 |
...
| J292.1 | J292 / S316 | WP8, WP10; live progress accounting | J291 | COMMITTED | EXPERIMENTAL | update the execution-refresh prose to avoid a commit-sensitive literal count and run the plan checker without adding fixtures | stable rebase/no-behind wording is present; plan check passes and no source or drawing bytes change |
...
| J319.1 | J319 / S343 | WP3.11-WP3.12, WP8, WP10; NURBSURFACE parser-state copy isolation | J318 | COMMITTED | EXPERIMENTAL | assert copied and assigned NURBSURFACE models accept a fresh subtype/typed-field walk after a partial coordinate parse while preserving public fields; keep all inputs in memory | focused `libdxfrw_hardening_tests` passes; no drawing bytes or derived fixtures |
| J320.1 | J320 / S344 | WP3.11-WP3.12, WP8, WP10; REVOLVEDSURFACE parser-state copy isolation | J319 | COMMITTED | EXPERIMENTAL | assert copied and assigned REVOLVEDSURFACE models treat a fresh class-ID group as the class ID after a partial source parse while preserving public fields; keep all inputs in memory | focused `libdxfrw_hardening_tests` passes; no drawing bytes or derived fixtures |
...
| J333.1 | J333 / S357 | WP3.11-WP3.12, WP8, WP10; DATALINK parser-state copy isolation | J332 | COMMITTED | EXPERIMENTAL | assert copied and assigned DATALINK models do not reuse stale body/custom-data/adapter/pending-handle state while preserving persisted connection metadata and custom rows; keep all inputs in memory | focused `libdxfrw_hardening_tests`, plan, fixture-admission, import-scope, sync, route, aggregate, support, release-readiness, speed, and diff gates; no drawing bytes or derived fixtures |
...
| J359.1 | J359 / S383 | WP5, WP8, WP10; durable advisory-evidence resolution | J358 | COMMITTED | NOT_APPLICABLE | add `metadata/external-evidence-resolution-v1.json` and a fail-closed checker with per-hash report aliases, `membershipBasis`, evidence date, magic/version, origin/provenance/admission disposition for all hashes, public URL or repository commit/path/blob, publisher/license state, audited ref tips, four distinct observed set digests, explicit no-membership/no-digest status for reconstructed J268, J284 joins, `archivedNonPromoting`, and `supersededBy`; reject support-state fields | checker negative/self-test/live and focused CTest pass; exact report/subset/digest, alias, provenance, ref-tip, repository-blob, J284/J295, admission, and non-promotion invariants are enforced; fixture admission and diff pass with no drawing payload |
| J359.2 | J359 / S383 | WP5, WP8, WP10; external runner corpus hygiene | J359.1 | COMMITTED | NOT_APPLICABLE | update `run_external_advisory.py` to identify DWG/DXF by magic, deduplicate by source SHA-256, and only then apply `--limit`; retain path aliases and emit separate path/unique-stream/DWG/non-DWG counts plus invalid/mislabeled rows and a generation receipt | synthetic duplicate, leading-comment/mislabeled DXF, invalid, binary/legacy magic, limit-order, deterministic-receipt, and timeout tests pass; live audit verifies 29 paths/28 unique streams/24 DWGs/four non-DWGs/four mislabeled paths; no output or drawing payload is committed |
| J360.1 | J360 / S384 | WP5, WP8, WP10; mandatory normalized dual-build schema v2 | J359 | COMMITTED | EXPERIMENTAL | compile one adapter/schema source against pinned target and standalone packages; bind target lock/archive plus adapter source/config/binary/static-link/build/interface digests; require typed finite input/identity/status, ordered callback, normalized record/field, handle/owner/block edge, opaque-carrier, unsupported-disposition, diagnostic, and first-failure fields even when arrays are empty, with stable IDs/cardinality/cross-reference checks | schema positives and adversarial mutations pass; both receipts prove the same exact 170-callback `DRW_Interface` set and effective build profile; absent/extra/ill-typed/non-finite or inconsistent callback, field, edge, carrier, disposition, diagnostic, provenance, format/version, and first-failure data fail closed |
| J360.2 | J360 / S384 | WP5, WP8, WP10; deterministic differential execution | J360.1 | COMMITTED | EXPERIMENTAL | replace placeholder commands with deterministic CMake build/run targets; verify byte-identical double execution; independently bind locked input and generated-output bytes, format, and version; enforce failed-write artifact absence; emit per-value `exact`, `toleranceNormalized`, `reviewedTargetDebt`, `excluded`, or `mismatch` outcomes with completeness/cardinality totals | clean Release build and all seven focused qualification CTests pass; four live target/standalone reports have zero mismatches and exact completeness, while unsupported-content blockers force `reviewedNonPromoting`; no external corpus refresh, broad suite, or committed drawing payload |
| J360.3 | J360 / S384a | WP5, WP8, WP10; exact expected-failure oracle | J360.2 | COMMITTED | EXPERIMENTAL | require each expected failure to resolve through one unique manifest case binding registered input/path, facade, direction, exact first-failure stage/code/path, and an output probe; reject side-equal, oracle-free, wrong, duplicate, or unregistered failure cases; exercise a deterministic existing-directory DXF write failure twice per side and preserve its marker | schema/runner and build-helper self-tests plus the focused live CTest pass; target and standalone both report `write`/2/`/output`, 146 exact values, zero mismatches, intact destination state, and a mandatory `reviewedNonPromoting` result; no drawing payload |
| J361.1 | J361 / S385 | WP5, WP8, WP10; admitted independent candidate fields | J360 | COMMITTED | EXPERIMENTAL | bind the three admitted J284 hashes to a pinned-source LibreDWG full-JSON build/dependency closure and identical J360 runs; assert separate RTEXT `recordClass=RTEXT↔entity=RTEXT`, normalized `handle↔handle`, and `text↔text_value` claims plus the exact MPOLYGON crosswalk `solid↔is_solid_fill`, `associative↔is_associative`, `name↔name`, `hpattern↔pattern_type`, `basePoint.z↔elevation`, `extPoint↔extrusion`, graph/preservation/error behavior, LARGE_RADIAL identity-only disposition, color/hatch-color exclusion, and ARCALIGNEDTEXT payload exclusion | 13 claims, three exact exclusion inventories, fixed `1e-12` tolerances, exact scoped diagnostic debt, sanitized/pinned oracle execution, runner/checker mutation tests, and 9/9 focused qualification tests pass; no external input, generated output, native, fixture-wide, or whole-version promotion is claimed |
| J362.1 | J362 / S386 | WP5, WP8, WP10; AC1024 extended-classes integrity | J360 | COMMITTED | EXPERIMENTAL | use the local-from-scratch writer to assert AC1024 `maxClassNum=1328`, class-string size `536312`, and actual high-bit extension; retain the RL-declared CRC position rather than the string-only buffer end; require a separate raw-page/footer parser and bitwise ODA CRC implementation that calls no production parser/CRC helper, stored/recomputed equality, `getClassesCrcMismatch()==0`, no `ClassesCrc` integrity entry, and `getIntegrityDiagnosticsDropped()==0` | independent test evidence binds RL 73839, bitSize 590705, max class 1328, footer high word 0x0010, and stored/recomputed CRC 0x443e; local roundtrip, reader-matrix, diagnostic, admitted-fixture, plan, fixture-policy, and diff gates pass; generated drawings remain temporary |
| J362.2 | J362 / S386a | WP5, WP8, WP10; AC1024 independent candidate fields | J362.1 | COMMITTED | EXPERIMENTAL | freeze `metadata/ac1024-candidate-fields-v1.json` before invoking the oracle; bind exact LibreDWG build/dependency closure, release/commit, command, platform, license, all 34 oracle discrepancy outcomes, and the J360 same-input report; accept only the exact named exclusions required by discoveries | contract …4764f0 validates 12/16 claims; exact exclusions are `rtext-insertion-{x,y,z}` because LibreDWG uses `pt[0..2]` and `rtext-rotation` because LibreDWG uses radians; J360 has 17,304 exact values and ten bound target-only diagnostic paths; `promotesSupport=false`, mutation gates and focused live CTest pass, and no generated payload is retained |
| J362.3 | J362 / S386b | WP5, WP8, WP10; corrected RTEXT candidate fields | J362.2 | COMMITTED | EXPERIMENTAL | create immutable v2 before invoking the oracle; map insertion `{x,y,z}` to LibreDWG `pt[0..2]`; normalize radians to degrees with the prederived `1e-12`-degree bound; rerun the pinned oracle and J360 on identical local bytes and bind every outcome | v2 digest …f667dd validates 16/16 with zero exclusions and no successor; exact input/oracle/closure, 34-discrepancy binding …400a, and J360 binding …63d are retained as metadata only; v1 stays byte-identical, all focused gates pass, and no generated payload is retained |
...
| J364.3 | J364 / S389 | WP8, WP10; hosted compiler portability repair | J364.1 | COMMITTED | DEFERRED_EXTERNAL | repair the GCC `-Werror=extra` enum/non-enum conditionals in `src/intern/dwgreader.cpp` with explicit `std::uint32_t` sentinel conversions and repair the MSVC local constexpr `kMatrixValueCount` lambda capture in `src/drw_objects.cpp`; run focused compile/tests, recompute the implementation digest/status, and invalidate prior native receipts without changing claims or fixtures | fresh Clang Release build and four focused tests pass; digest `d8cae87cd5c72469d5aba2897230437d8c4285419b2c0bf2c2956f85d5cb281c` recomputed over 165 inputs; claims digest and support rows unchanged; no fixture or claim bytes changed; plan, workflow, support, fixture, and diff gates pass; committed in S389; digest is historical and is superseded by S390 after the next source repair |
| J364.4 | J364 / S390 | WP8, WP10; hosted GCC warning repair | J364.3 | COMMITTED | DEFERRED_EXTERNAL | repair the `-Werror=misleading-indentation` diagnostics in the ACIS bounds lambda without changing geometry semantics; recompute the implementation digest/status and invalidate prior receipts without changing claims or fixtures | split coordinate-bound conditionals; focused Clang build and four fast tests pass; digest `a0960bffed1b544ca86d474b2f91b5a2dcbd8c50c975360757925e14c4fa6a89` was committed in S390 and is superseded by S391 after the next source repair; no fixture or claim bytes changed |
| J364.5 | J364 / S391 | WP8, WP10; hosted GCC/MSVC warning repair | J364.4 | COMMITTED | DEFERRED_EXTERNAL | repair the remaining GCC enum/non-enum, deprecated-copy, and ignored-qualifier diagnostics in `src/drw_entities.cpp`, `src/drw_entities.h`, and `src/drw_objects.cpp` while preserving behavior; recompute the implementation digest/status and invalidate prior receipts without changing claims or fixtures | replacement run `35177810798` builds the repaired library and focused targets; digest `55ea027c241237450dde7ccc215245ed596ef7564c8dcf4c65e3e4a87578dc4f` is historical and superseded by later repair slices; no fixture or claim bytes changed |
| J364.6 | J364 / S392 | WP8, WP10; hosted handle/test-seam repair | J364.5 | COMMITTED | DEFERRED_EXTERNAL | replace private reader-matrix access with the public `readBuffer` path and concrete adapter; convert remaining `DRW::NoHandle` sentinel conditionals to explicit unsigned values; recompute digest/status and invalidate prior receipts without changing claims or fixtures | replacement run `35178718521` builds the focused targets; Linux raw-replay ordering was the only remaining focused failure; digest `3230cb388a2aeff7219533a1f7fcff3af89f04657b208c2620c4ee70829ff32e` is superseded by S393; no fixture or claim bytes changed |
...
| J364.9 | J364 / S395 | WP8, WP10; Windows writer diagnostic capture | J364.8 | COMMITTED | DEFERRED_EXTERNAL | print `dwgRW::getLastDiagnostic()` code/message for each failed local DWG write so the next Windows repair targets the actual failure stage; keep behavior, claims, and fixtures unchanged and recompute the digest/status | diagnostic run `35181235487` isolates every Windows failure to `open-failure` before writer emission; no fixture or claim bytes changed; superseded by S396 and later repair slices |
