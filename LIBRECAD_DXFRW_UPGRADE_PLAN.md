...
- Latest implementation slice: S18/I0.2e-C DWG direct/journal compound
  delivery identity now binds the mapped-entity immediate/journal contract,
  journal event replay, block-scope transaction replay and lease retirement,
  and `readDwgBlocks`’ version-gated journal path alongside its direct
  `addBlock`/entity-walk/`endBlock` fallback. This keeps receipt/staging,
  journal replay, direct delivery, and compound dispositions distinct instead
  of collapsing them into `entryParse`. It builds on the committed table,
  compound-transition, and raw-section/object lifecycle slices plus the DXF
  proof; remaining work is the detailed BLOCK/ENDBLK ownership/reachability
  closure, reader lifecycle roll-up, mapping, and format qualification. The
  slice is source-only, fail-closed, and self-checked before commit.
...
- Active work: I0.2b exact parser-publication/callback proof, with its raw
  ...
  I0.2e-B binds compound entity case arms to versioned staging helpers; and
  I0.2e-C now binds direct/journal delivery and block-scope replay ownership.
...
| S18 | I0: deterministic both-façade parity inventory | S17 | ACTIVE | pinned generator inputs, source/public-surface and pipeline-edge extraction, cardinality-aware mapping, zero-unmapped/duplicate checks, deterministic `--check` | source/package integration, raw-publication terminal proof, read-side raw eligibility, DXF ASCII/binary/R12 transport selection, DXF raw writer/replay transforms, DWG raw replay ingress/preflight/internal-registration/owner-bookkeeping identity, DWG raw section ingress/buffer/finalizer identity, all-nine-descriptor table receipt/parse/map identity, compound ATTRIB/SEQEND/INSERT/MINSERT/VERTEX/POLYLINE transition identity, and direct/journal block delivery identity are committed; target and standalone routes include exact carrier/callback/guard/branch/predecessor metadata plus ordered DWG replay/table/compound/delivery edges, while detailed DWG BLOCK/ENDBLK ownership/reachability, reader lifecycle roll-up, and mapping remain before this becomes a parity claim; no drawing payloads | S19 |
