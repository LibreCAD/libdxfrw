...
dxfRW::dxfRW(const char* name)
    : m_handleAllocator(std::make_unique<HandleAllocator>()) {
...
bool dxfRW::reserveHandle(std::uint32_t h) {
    try {
...
std::uint32_t dxfRW::highWaterHandle() const {
    return m_handleAllocator->current();
}
...
bool dxfRW::processClasses() {
    ...
    while (reader->readRec(&code)) {
        ...
        case 90:
            ...
            // LibreDWG sets the upper 16 bits even though this is a 16-bit mask.
...
bool dxfRW::processDimStyle() {
    ...
    struct DimstyleNameScope {
        dxfReader& reader;
        explicit DimstyleNameScope(dxfReader& r) : reader(r) {
            reader.setAllowDimstyleNames(true);
        }
...
bool dxfRW::processBlock() {
    ...
    auto consumeEndBlock = [&]() -> bool {
        ...
        while (reader->readRec(&code)) {
            ...
            // ENDBLK needs an owner when BLOCK carries a BLOCK_RECORD owner,
            // but legacy writers may use a different (model-space) owner.
...
bool dxfRW::processObjects() {
    ...
    //"First DICTIONARY in this section" is one of the two tests that identify
    //the root, so it is scoped to the section, not to the file.
    m_objectsDictionarySeen = false;
    int code;
    if (!reader->readRec(&code)) {
...
bool dxfRW::processSortEntsTable() {
    ...
    while (reader->readRec(&code)) {
        ...
        const bool sortHandle = data.m_dxfInBody
            && code == DRW::dxfCode::HANDLE;
        if (sortHandle
                ? !(captureRawGroup(raw, code, /*validateHandles=*/true,
                                    /*selfHandle=*/false)
...
/// Whether a DICTIONARY just read is the root NamedObjectsDictionary.
///
/// The root is regenerated by the writer at its own fixed handle, so routing
/// the source's copy into the raw net would emit a second one. Everything else
/// has to be routed, or it is destroyed by any consumer that does not model
/// dictionaries.
///
/// DRW_RawDxfObject::parentHandle cannot answer this on its own. It is
/// value-initialised to 0 and assigned only when an owner group is read, so
/// "carried no code 330" and "code 330 named handle 0" are the same value --
/// and code 330 is optional on a DICTIONARY. R2000-era writers routinely omit
/// it: of 1840 real DXFs, 284 hold a dictionary with no owner group, and
/// treating each as the root dropped 1134 DICTIONARY and 283
/// ACDBDICTIONARYWDFLT records with nothing said.
///
/// So ask the two questions that do discriminate. The fixed handles C and D
/// are what the writer regenerates. And the DXF specification puts the named
/// object dictionary first in the OBJECTS section, which is what
/// `firstInObjects` reports -- a dictionary that is neither is somebody's
/// named dictionary, whatever its owner group says.
bool isRootDictionary(const DRW_RawDxfObject &raw, bool firstInObjects) {
    if (raw.handle == 0xCu || raw.handle == 0xDu)
        return true;
    return firstInObjects && raw.parentHandle == 0;
}
...
bool dxfRW::processDictionary() {
    ...
    // DXF->DXF (re-attached to the regenerated root via setRootDictEntries).
    // The source root is not routed: it is identified by fixed handle C/D or
    // by being the first regular DICTIONARY with no owner group.  An absent
    // owner group alone is not enough to classify a record as the root.
    DRW_RawDxfObject raw;
    raw.name = nextentity;
    while (reader->readRec(&code)) {
        DRW_DBG(code); DRW_DBG("\n");
        if (0 == code) {
            ...
            //Skip only the root — the codec regenerates it, and routing it
            //would duplicate the NamedObjectsDictionary / ACAD_GROUP dict.
            const bool first = !m_objectsDictionarySeen;
            m_objectsDictionarySeen = true;
            if (!isRootDictionary(raw, first))
                iface->addRawDxfObject(raw);
            return true;  //found new entity or ENDSEC, terminate
        }
...
bool dxfRW::processDictionaryWithDefault() {
    ...
    while (reader->readRec(&code)) {
        ...
        if (0 == code) {
            ...
            // WDFLT is a separate object kind.  It must not consume the
            // first-regular-DICTIONARY slot used for owner-less root
            // detection; otherwise a WDFLT appearing first would make the
            // actual root look like a named dictionary.
            if (!isRootDictionary(raw, false))
                iface->addRawDxfObject(raw);
            return true;  //found new entity or ENDSEC, terminate
        }
