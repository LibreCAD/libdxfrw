...
public:
    ...
    enum TYPE type;
public:
    dxfReader(std::istringstream *stream){
        filestr = stream;
        type = INVALID;
    }
    virtual ~dxfReader(){}
...
public:
    dxfReaderBinary(std::istringstream *stream):dxfReader(stream){skip = false; }
...
public:
    dxfReaderAscii(std::istringstream *stream):dxfReader(stream){skip = true; }
